...
这里有一个相对的顺序号、确认号，和一个原始的顺序号、确认号。我们考虑这样一种情况，假设三次握手时，客户端发送的SYN包的顺序号是0，但此时网络拥堵，导致连接没有建立成功。客户端只好再起发起握手，尝试连接，如果这时候网络通畅了，服务器收到了客户端上次发送的包，并以此为依据建立连接，则会导致连接出现异常。假设两次连接请求发送的数据是不一样的，建立的连接就可能以旧的数据为准。因此，为避免这种情况的发生，建立TCP连接时的初始顺序号必须是随机的，客户端在收到服务器的ACK包时就可以根据顺序号判读是否该继续建立连接。随机的顺序号不好辨认，所以Wireshark给我们提供了一个相对的顺序号，方便我们追踪请求。为了方便追踪，我们关注的也是相对序号，此包的相对顺序号为0，相对确认号也为0。（如无特别说明，后面说的顺序号为相对顺序号，确认号为相对确认号。）
